            ...
            match change_result {
                Ok(_) => {
                    ...
                    if copy_to_clipboard(password_as_string.deref()).is_err() {
                        println_ok!("Hmm, we tried to copy your changed password to your clipboard, but something went wrong. Don't worry, it's saved, and you can see it with `rooster get {} --show`", app_name);
                    } else {
